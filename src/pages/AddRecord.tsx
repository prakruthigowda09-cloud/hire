  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center py-20 space-y-6 text-center">
        ...
        <div>
          <h2 className="text-3xl font-bold text-stone-900">Record Added Successfully!</h2>
          <p className="text-stone-500 mt-2">Redirecting you to the records table...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-stone-900">New Candidate Record</h1>
          <p className="text-stone-500">Add details for a new drive candidate</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-3xl border border-stone-200 shadow-sm space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-stone-100 pt-6">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <div className="w-full px-4 py-4 bg-stone-50 border-2 border-dashed border-emerald-200 rounded-xl flex items-center justify-center gap-2 text-emerald-600 hover:bg-emerald-50 transition-colors">
                <Upload size={18} />
                <span className="font-medium">{fileNames.resume || 'Click to Upload Resume'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <div className="w-full px-4 py-4 bg-stone-50 border-2 border-dashed border-emerald-200 rounded-xl flex items-center justify-center gap-2 text-emerald-600 hover:bg-emerald-50 transition-colors">
                <Upload size={18} />
                <span className="font-medium">{fileNames.offer_letter || 'Click to Upload Offer Letter'}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="pt-6 flex gap-4">
          ...
          >
            {loading ? 'Saving...' : 'Create Candidate Record'}
          </button>
          <button
            type="button"
            onClick={() => navigate(-1)}
